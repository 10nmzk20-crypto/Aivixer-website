...
Life Design ViXer の Web マーケティング分析ツール。**5 つのツールの数字を手で入れると、担当 5 人が分析し、今月やるべきことが最大 3 つ出ます。**

入力から確認まで 5〜10 分で終わる形にしてあります。外部 AI（Claude / OpenAI）は呼びません。分析はすべてアプリ内の計算です。

## 担当 5 人（1 ツール 1 担当）

| 担当 | 見るツール | 答える問い |
|---|---|---|
| Search Console 担当 | Google Search Console | どんな言葉で検索され、何位だったか |
| Google ビジネスプロフィール担当 | Google ビジネスプロフィール | 地図で見つけてもらえたか |
| GA4 担当 | Google Analytics 4 | 何人来て、どこを見て、何を押したか |
| Clarity 担当 | Microsoft Clarity | なぜそこで止まったか |
| hacomono 担当 | hacomono・受付 | 実際に予約・入会したか |

各担当は自分のツールの数字だけを見て、**現状・傾向・問題点・改善案**を出します。他のツールの数字は根拠に使いません。

最後にアプリが 5 人分をまとめ、**今月やるべきこと**を最大 3 つに絞ります。集客の流れの上流（見つけてもらう側）で詰まっている場所から選び、同じ担当の指摘ばかりにならないようにしています。

判定はすべて `src/analysis/tool-review.ts` に書いてあります。同じ数字を入れれば毎回同じ結果になり、なぜそう言えるのかを後から説明できます。動作確認は `npm run review:check`。

## 画面は 3 つだけ

1. **ダッシュボード** … 前回の分析と、担当 5 人
2. **新しい分析** … 5 つのツールの数字を入れる
3. **分析結果** … 今月やるべきこと → 担当ごとの分析 → （任意）ChatGPT 用レポート

## 今は作っていないもの

API 連携・自動取得・施策管理・タスク管理・ナレッジは入れていません。まず「数字を入れる → 分析される → 次に何をすべきか分かる」を完成させるためです。

施策管理の画面と API は外していますが、**コードとデータベースの表は残してあります**（`src/api/tasks.ts`、`src/workflows/` 以下）。再開するときは `src/api/app.ts` で route を戻します。
...
└── src/                   Worker
    ...
    ├── db/repo.ts         D1 の読み書き
    ├── employees/         AI 社員 6 人の定義と役割プロンプト
    ├── policy/            承認が必須の行為の判定
