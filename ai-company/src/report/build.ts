...
export function buildReport(src: ReportSources): string {
  ...
  if (src.pastTasks.length === 0) L.push(NA);
    ...
    for (const t of src.pastTasks) {
      ...
      if (t.laozi && t.sunzi && t.confucius) L.push(`    3 軸: 老子 ${t.laozi} / 孫子 ${t.sunzi} / 孔子 ${t.confucius}`);
...
経営判断では、
老子・孫子・孔子の3つの思想軸を使ってください。

老子：
人が頑張り続けなくても自然に回る仕組みか。
無駄な仕事を減らし、
一度作れば繰り返し働く仕組みを優先してください。

孫子：
競合と正面衝突せず、
ViXerが優位に立てる場所を選んでいるか。
価格競争や大型ジムと同じ土俵を避けてください。

孔子：
短期利益より、
顧客・社員・会社の信頼を積み上げる判断か。
誠実さと長期的なブランド価値を重視してください。
